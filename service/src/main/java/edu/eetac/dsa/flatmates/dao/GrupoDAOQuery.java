...
public interface GrupoDAOQuery {
    ...
    public final static String GET_USER = "select hex(g.grupoid) as grupoid, hex(g.userid) as userid, g.puntos, u.loginid from grupousuario g, users u where grupoid = UNHEX(?) and u.id=g.userid order by g.puntos desc";
