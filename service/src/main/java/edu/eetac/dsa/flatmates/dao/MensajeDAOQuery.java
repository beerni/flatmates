...
public interface MensajeDAOQuery {
    ...
    public final static String GET_MENSAJES = "select hex(m.id) as id, hex(m.userid) as userid, m.subject, m.mensaje, m.creation_timestamp, m.last_modified, u.loginid from mensaje m, users u where u.id=m.userid order by m.creation_timestamp desc limit 5";
