...
public interface MensajeDAOQuery {
    ...
    public final static String GET_MENSAJE_BY_ID = "select hex(m.id) as id, hex(m.userid) as userid, m.subject, m.mensaje, m.creation_timestamp, m.last_modified, u.loginid from mensaje m, users u where t.id=unhex(?) and u.id = m.userid";
