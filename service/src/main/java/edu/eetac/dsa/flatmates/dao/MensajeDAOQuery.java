...
public interface MensajeDAOQuery {
    ...
    public final static String GET_MENSAJES_AFTER = "select hex(m.id) as id, hex(m.userid) as userid, m.content, m.creation_timestamp, m.last_modified, u.loginid from mensaje m, users u where u.id=m.userid order by m.creation_timestamp desc limit ?,5";
