...
public interface MensajeGrupoDAOQuery {
    ...
    public final static String GET_MENSAJE_BY_ID = "select hex(id) as id,hex(grupoid) as grupoid, hex(userid) as userid, subject, content, creation_timestamp, last_modified from mensajegrupo where id=unhex(?)";
    public final static String GET_MENSAJES = "select hex(id) as id, hex(grupoid) as grupoid, hex(userid) as userid, subject, creation_timestamp, last_modified from mensajegrupo where grupoid=unhex(?)";
