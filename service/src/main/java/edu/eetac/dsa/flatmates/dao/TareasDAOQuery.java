...
public class TareasDAOQuery {
    ...
    public final static String GET_TAREA_BY_ID = "select hex(id) as id, hex(userid) as userid, tarea, imagen, punts from tareas where id=unhex(?) and grupoid = unhex(?)";
    ...
    public final static String GET_TAREA = "select hex(id) as id, hex(userid) as userid, tarea, punts from tareas where grupoid = unhex(?)";
    public final static String UPDATE_IMAGE = "update tareas set imagen=? ";
